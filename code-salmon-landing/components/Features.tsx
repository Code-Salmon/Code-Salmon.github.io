...
export default function Features() {
  return (
    <section className="text-text py-12 sm:py-20 px-4 sm:px-6 font-body">
      <h2 className="text-2xl sm:text-3xl font-bold font-subheader text-center mb-8 sm:mb-12 text-white">
        Spec Ops Toolkit
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
        {features.map((f) => (
          <div
            key={f.title}
            className="relative bg-white text-gray-800 pl-8 sm:pl-12 font-subheader border border-gray-300 p-4 sm:p-6 shadow-md overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl"
          >
            {/* Red margin line */}
            <div className="absolute top-0 bottom-0 left-6 sm:left-10 w-[2px] bg-red-400 opacity-40 z-0" />
            ...
            <div className="relative z-10">
              ...
              <p className="text-xs sm:text-sm">{f.desc}</p>
            </div>
          </div>
        ))}
      </div>
