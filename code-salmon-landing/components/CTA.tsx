...
export default function CTA() {
  const team = [
    ...
    },
    {
      ...
      role: 'Software Engineer',
      image: '/images/jamie.png',
      linkedin: 'https://www.linkedin.com/in/jamiejtait/',
      github: 'https://github.com/jamjota',
    },
  ];

  return (
    <section className='text-white text-center py-12 sm:py-20 px-4 sm:px-6'>
      <h2 className='text-2xl sm:text-3xl text-salmon font-subheader font-bold mb-4'>
        Prevent Silent Assassins from Killing your API Responses!
      </h2>
      <p className='text-base sm:text-lg mb-8 sm:mb-12 font-subheader max-w-xl mx-auto'>
        CLI Gadget Creators:
      </p>
      <div className='grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 max-w-6xl mx-auto'>
        {team.map((person) => (
          <div key={person.name} className='text-center'>
            <a href={person.linkedin} target='_blank' rel='noopener noreferrer'>
              <img
                src={person.image}
                alt={person.name}
                className='w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-full object-cover shadow-md mb-3 border-2 border-white hover:opacity-80 transition'
              />
            </a>
